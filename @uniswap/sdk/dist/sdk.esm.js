...
(function (ChainId) {
  ...
  ChainId[ChainId["NEBULA"] = 107] = "NEBULA";
})(ChainId || (ChainId = {}));
...
var FACTORY_ADDRESS = '0x9550b0c83AD5a58898cD4267987Af67e7E52bF55';
var INIT_CODE_HASH = '0x6e898add6d04d6443c8d2544bc82ca8351779bf4ae92a6e7a5d313be8603ea07';
...
Currency.FANTOM = /*#__PURE__*/new Currency(18, 'FTM', 'Fantom Opera');
Currency.NBX = /*#__PURE__*/new Currency(18, 'NBX', 'Nebula X');
var ETHER = Currency.ETHER;
var FANTOM = Currency.FANTOM;
var NBX = Currency.NBX;
...
var WETH = (_WETH = {}, _WETH[ChainId.NOVA] = /*#__PURE__*/new Token(ChainId.NOVA, '0x657a66332A65B535Da6C5d67b8cD1D410c161a08', 18, 'WSNT', 'Wrapped SNT'), _WETH[ChainId.FANTOM] = /*#__PURE__*/new Token(ChainId.FANTOM, '0x21be370d5312f44cb42ce377bc9b8a0cef1a4c83', 18, 'WFTM', 'Wrapped FTM'), _WETH[ChainId.NEBULA] = /*#__PURE__*/new Token(ChainId.NEBULA, '0x7e2D6Ac6cEAAa20a3Cc4CA4c1b0dFC6b95332A9d', 18, 'WNBK', 'Wrapped NBK'), _WETH);
...
var CurrencyAmount = /*#__PURE__*/function (_Fraction) {
  ...
  CurrencyAmount.fantom = function fantom(amount) {
    return new CurrencyAmount(FANTOM, amount);
  };

  CurrencyAmount.nebula = function nebula(amount) {
...
var Pair = /*#__PURE__*/function () {
  function Pair(tokenAmountA, tokenAmountB) {
    ...
    this.liquidityToken = new Token(tokenAmounts[0].token.chainId, Pair.getAddress(tokenAmounts[0].token, tokenAmounts[1].token), 18, 'NLT', 'Nova Liquidity Token');
...
var Route = /*#__PURE__*/function () {
  function Route(pairs, input, output) {
    ...
    !(input instanceof Token && pairs[0].involvesToken(input) || (input === ETHER || input === FANTOM || input === NBX) && pairs[0].involvesToken(WETH[pairs[0].chainId])) ? process.env.NODE_ENV !== "production" ? invariant(false, 'INPUT') : invariant(false) : void 0;
    !(typeof output === 'undefined' || output instanceof Token && pairs[pairs.length - 1].involvesToken(output) || (output === ETHER || output === FANTOM || output === NBX) && pairs[pairs.length - 1].involvesToken(WETH[pairs[0].chainId])) ? process.env.NODE_ENV !== "production" ? invariant(false, 'OUTPUT') : invariant(false) : void 0;
...
function wrappedAmount(currencyAmount, chainId) {
  ...
  if (currencyAmount.currency === ETHER && chainId === ChainId.NOVA || currencyAmount.currency === FANTOM && chainId === ChainId.FANTOM || currencyAmount.currency === NBX && chainId === ChainId.NEBULA) return new TokenAmount(WETH[chainId], currencyAmount.raw);
   process.env.NODE_ENV !== "production" ? invariant(false, 'CURRENCY') : invariant(false) ;
}

function wrappedCurrency(currency, chainId) {
  if (currency instanceof Token) return currency;
  if (currency === ETHER && chainId === ChainId.NOVA || currency === FANTOM && chainId === ChainId.FANTOM || currency === NBX && chainId === ChainId.NEBULA) return WETH[chainId];
   process.env.NODE_ENV !== "production" ? invariant(false, 'CURRENCY') : invariant(false) ;
}
/**
...
var Trade = /*#__PURE__*/function () {
  function Trade(route, amount, tradeType) {
    ...
    this.inputAmount = tradeType === TradeType.EXACT_INPUT ? amount : route.input === ETHER && route.chainId === ChainId.NOVA ? CurrencyAmount.ether(amounts[0].raw) : route.input === FANTOM && route.chainId === ChainId.FANTOM ? CurrencyAmount.fantom(amounts[0].raw) : route.input === NBX && route.chainId === ChainId.NEBULA ? CurrencyAmount.nebula(amounts[0].raw) : amounts[0];
    this.outputAmount = tradeType === TradeType.EXACT_OUTPUT ? amount : route.output === ETHER && route.chainId === ChainId.NOVA ? CurrencyAmount.ether(amounts[amounts.length - 1].raw) : route.output === FANTOM && route.chainId === ChainId.FANTOM ? CurrencyAmount.fantom(amounts[amounts.length - 1].raw) : route.output === NBX && route.chainId === ChainId.NEBULA ? CurrencyAmount.nebula(amounts[amounts.length - 1].raw) : amounts[amounts.length - 1];
  ...
  _proto.minimumAmountOut = function minimumAmountOut(slippageTolerance, chainId) {
    ...
    if (this.tradeType === TradeType.EXACT_OUTPUT) {
      ...
      return this.outputAmount instanceof TokenAmount ? new TokenAmount(this.outputAmount.token, slippageAdjustedAmountOut) : chainId === ChainId.FANTOM ? CurrencyAmount.fantom(slippageAdjustedAmountOut) : chainId === ChainId.NEBULA ? CurrencyAmount.nebula(slippageAdjustedAmountOut) : CurrencyAmount.ether(slippageAdjustedAmountOut);
    }
  }
  /**
  ...
  _proto.maximumAmountIn = function maximumAmountIn(slippageTolerance, chainId) {
    ...
    if (this.tradeType === TradeType.EXACT_INPUT) {
      ...
      return this.inputAmount instanceof TokenAmount ? new TokenAmount(this.inputAmount.token, slippageAdjustedAmountIn) : chainId === ChainId.FANTOM ? CurrencyAmount.fantom(slippageAdjustedAmountIn) : chainId === ChainId.NEBULA ? CurrencyAmount.nebula(slippageAdjustedAmountIn) : CurrencyAmount.ether(slippageAdjustedAmountIn);
    }
  }
  /**
...
var Router = /*#__PURE__*/function () {
  ...
  Router.swapCallParameters = function swapCallParameters(trade, options) {
    var etherIn = trade.inputAmount.currency === ETHER || trade.inputAmount.currency === FANTOM || trade.inputAmount.currency === NBX;
    var etherOut = trade.outputAmount.currency === ETHER || trade.outputAmount.currency === FANTOM || trade.outputAmount.currency === NBX; // the router does not support both ether in and out
...
export { ChainId, Currency, CurrencyAmount, ETHER, FACTORY_ADDRESS, FANTOM, Fetcher, Fraction, INIT_CODE_HASH, InsufficientInputAmountError, InsufficientReservesError, MINIMUM_LIQUIDITY, NBX, Pair, Percent, Price, Rounding, Route, Router, Token, TokenAmount, Trade, TradeType, WETH, currencyEquals, inputOutputComparator, tradeComparator };
